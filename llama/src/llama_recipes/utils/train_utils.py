...
def train(model, train_dataloader, eval_dataloader, tokenizer, optimizer, lr_scheduler, gradient_accumulation_steps, train_config, fsdp_config=None, local_rank=None, rank=None):
    ...
    res = {'step': [], 'loss': []}
    save_name_pre = 'name_{}_bsz_{}_opt_{}'.format(file_name, train_config.batch_size_training, train_config.opt, train_config.rho)
    ...
    for epoch in range(train_config.num_epochs):
        ...
        with MemoryTrace() as memtrace:  # track the memory usage
            ...
            for step, batch in enumerate(train_dataloader):
                cur_iter = step + epoch * len(train_dataloader)
        ...
        train_perplexity = torch.tensor([0.0])
        ...
        if train_config.run_validation and (epoch==train_config.num_epochs-1):
            ...
            if train_config.save_model:
                if train_config.enable_fsdp:
                    ...
                    print("Saving model (single-GPU mode)...")
                    if train_config.use_peft:
                        model.save_pretrained(train_config.output_dir)
                        print(f"PEFT modules saved to {train_config.output_dir}")
                    else:
                        state_dict = model.state_dict()
                        output_path = os.path.join(train_config.output_dir, "opt_{}_model.pth".format(train_config.opt))
                        torch.save(state_dict, output_path)
                        print(f"Full model state_dict saved to {output_path}")

                        if train_config.save_optimizer:
                            optim_output_path = os.path.join(train_config.output_dir, f"epoch_{epoch}_optimizer.pth")
                            torch.save(optimizer.state_dict(), optim_output_path)
                            print(f"Optimizer state_dict saved to {optim_output_path}")
...
def save_train_params(train_config, fsdp_config, rank):
    ...
    file_name = os.path.join(save_dir,'train_params_{}.yaml'.format(train_config.opt))
